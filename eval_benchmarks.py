...
from src.mambalitesr.experiment_manager import ExperimentManager, get_latest_experiment_dir


def evaluate_dataset(model: torch.nn.Module, dataset_path: Path, device: torch.device, scale: int) -> tuple[float, float]:
...
def main() -> None:
    ...
    print("-" * 60)
    print(f"{'Dataset':<10} {'Your PSNR':<10} {'Baseline':<10} {'Diff':<8} {'Your SSIM':<10} {'Baseline':<10} {'Diff':<8}")
    print("-" * 60)
    ...
    for dataset_name in datasets.keys():
        if results[dataset_name][0] > 0:  # Only compare if we have results
            ...
            print(f"{dataset_name:<10} {your_psnr:>9.2f} {base_psnr:>9.2f} "
    ...
    if valid_results:
        ...
        baseline_avg_psnr = sum(baseline_results[name][0] for name in results.keys() if results[name][0] > 0) / len(valid_results)
