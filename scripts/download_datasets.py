...
def download_with_progress(url: str, dest_path: Path) -> None:
    """Download a file with tqdm progress bar using requests with urllib fallback."""
    try:
        # Try with requests first (more reliable with SSL issues)
        ...
        print(f"Requests failed ({e}), trying urllib with SSL bypass...")
        # Fallback to urllib with SSL context
        import urllib.request
        opener = urllib.request.build_opener(urllib.request.HTTPSHandler(context=ssl_context))
        urllib.request.install_opener(opener)
        
        with tqdm(unit='B', unit_scale=True, unit_divisor=1024, miniters=1, desc=dest_path.name) as pbar:
            def update_progress(block_num: int, block_size: int, total_size: int) -> None:
                if pbar.total is None and total_size > 0:
                    pbar.total = total_size
                pbar.update(block_size)
            
            urlretrieve(url, dest_path, reporthook=update_progress)
...
def maybe_populate_df2k_from_div2k(root: Path) -> None:
    ...
    if dir_has_files(df2k_hr):
        print("DF2K/HR already populated, skipping.")
        return
...
def main() -> None:
    ...
    if not (root / "DIV2K_train_HR").exists():
        ...
        download_with_progress(DIV2K_TRAIN_URL, div2k_train_zip)
        safe_zip_extract(div2k_train_zip, root)
        div2k_train_zip.unlink(missing_ok=True)
    else:
        print("DIV2K train HR already exists, skipping download.")
        
    if not (root / "DIV2K_valid_HR").exists():
        print("Downloading DIV2K valid HR...")
        download_with_progress(DIV2K_VALID_URL, div2k_valid_zip)
        safe_zip_extract(div2k_valid_zip, root)
        div2k_valid_zip.unlink(missing_ok=True)
    else:
        print("DIV2K valid HR already exists, skipping download.")

    # DF2K expected layout and population
    df2k_hr = root / "DF2K" / "HR"
    ensure_dir(df2k_hr)
